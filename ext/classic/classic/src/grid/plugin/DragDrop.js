/**
 * This plugin provides drag and/or drop functionality for a {@link Ext.grid.View grid}.
 *
 * It creates a specialized instance of {@link Ext.dd.DragZone DragZone} which knows how to drag out of a {@link
 * Ext.grid.View grid} and loads the data object which is passed to a cooperating {@link Ext.dd.DragZone DragZone}'s
 * methods with the following properties:
 *
 * - `copy` : {@link Boolean}
 *
 *   The value of the {@link Ext.grid.View grid}'s `copy` property, or `true` if the grid was configured with `allowCopy: true` _and_
 *   the control key was pressed when the drag operation was begun.
 *
 * - `view` : {@link Ext.grid.View grid}
 *
 *   The source grid from which the drag originated.
 *
 * - `ddel` : HTMLElement
 *
 ...
 * - `item` : HTMLElement
 *
 *   The grid node upon which the mousedown event was registered.
 *
 * - `records` : {@link Array}
 *
 *   An Array of {@link Ext.data.Model Model}s representing the selected data being dragged from the source {@link Ext.grid.View grid}.
 *
 ...
 *
 * Adding this plugin to a view means that two new events may be fired from the client grid, `{@link #beforedrop
...
Ext.define('Ext.grid.plugin.DragDrop', {
    ...
    /**
     * @event beforedrop
     * **This event is fired through the grid. Add listeners to the {@link Ext.grid.View grid} object**
     *
     * Fired when a drop gesture has been triggered by a mouseup event in a valid drop position in the grid.
     * 
     ...
     *
     * @param {HTMLElement} node The {@link Ext.grid.View grid} node **if any** over which the mouse was positioned.
     *
     ...
     * properties:
     * @param {Boolean} data.copy The value of the grid's `copy` property, or `true` if the grid was configured with
     * `allowCopy: true` and the control key was pressed when the drag operation was begun
     * @param {Ext.grid.View} data.view The source grid from which the drag originated.
     * @param {HTMLElement} data.ddel The drag proxy element which moves with the mouse
     * @param {HTMLElement} data.item The grid node upon which the mousedown event was registered.
     * @param {Ext.data.Model[]} data.records An Array of {@link Ext.data.Model Model}s representing the selected data being
     * dragged from the source grid.
     *
    ...
    /**
     * @event drop
     * **This event is fired through the grid. Add listeners to the grid object** Fired when a drop operation
     * has been completed and the data has been moved or copied.
     *
     * @param {HTMLElement} node The grid node **if any** over which the mouse was positioned.
     *
     ...
     * - `copy` : {@link Boolean}
     *
     *   The value of the grid's `copy` property, or `true` if the grid was configured with `allowCopy: true` and
     *   the control key was pressed when the drag operation was begun
     *
     * - `view` : {@link Ext.grid.View grid}
     *
     ...
     * - `item` : HTMLElement
     *
     *   The {@link Ext.grid.View grid}{@link Ext.grid.View grid} node upon which the mousedown event was registered.
     *
     * - `records` : {@link Array}
     *
     *   An Array of {@link Ext.data.Model Model}s representing the selected data being dragged from the source grid.
     *
     * @param {Ext.data.Model} overModel The Model over which the drop gesture took place.
     *
     * @param {String} dropPosition `"before"` or `"after"` depending on whether the mouse is above or below the midline
